(function (run) {
  ...
  const saludar = (nombre) =>
    console.log(`Hola ${nombre}, Se bienvenido a esta clase 🚀.`);
  ...
  console.log(
    `El resultado de la multiplicación de ${A} * ${B} es ${multiplicar(
      A,
      B
    )} 😍`
  );
...
(function (run) {
  ...
  console.log(
    `${obtFechaHoraActual()} ⭐ Iniciaremos el procedimeinto de envio del mensaje...`
  );

  enviarMensajeAsincrono(
    "Hola Ezequiel, Cómo te va con las clases del curso de backend?"
  );

  console.log(
    `${obtFechaHoraActual()} ⭐ Finalizamos el procedimeinto de envio del mensaje.`
  );
})(false);
...
(function (run) {
  ...
  enviarMensajeAsincrono(
    "Hola Manuel, Cómo te va con las clases del curso de backend?"
  );
...
(function (run) {
  ...
  const ejecutarOperacion = (a, b, ope) => {
    console.log(
      `Ejecutando una nueva operación con los valores ${a} y ${b}...`
    );
    return ope(a, b);
  };
...
(function (run) {
  ...
  division(18, 6).then(callbackExito).catch(callbackFallo);

  division(3, 0).then(callbackExito).catch(callbackFallo);
})(false);
...
(function (run) {
  ...
  cuentaRegresiva(5).then(() => console.log("Lanzamiento... 🚀"));
})(false);
...
(function (run) {
  ...
  new Promise((resolve) => {
    console.log(
      `${obtFechaHoraActual()} 🛒 Intentando reservar los artículos.`
    );
    setTimeout(() => {
      console.log(
        `${obtFechaHoraActual()} 🛒 Reservación de artículos éxitosa.`
      );
      resolve();
    }, 1000);
  })
    .then(() => {
      return new Promise((resolve, reject) => {
        ...
        setTimeout(() => {
          ...
          console.log(
            `${obtFechaHoraActual()} 💸 Ejecutado pago del artículos con éxitosa.`
          );
          resolve();
        }, 1000);
      });
    ...
    .then(() => {
      return new Promise((resolve) => {
        console.log(
          `${obtFechaHoraActual()} 👁‍🗨 Notificando a los proveedores de los artículos.`
        );
        setTimeout(() => {
          console.log(
            `${obtFechaHoraActual()} 👁‍🗨 Notificación a proveedores éxitosa.`
          );
          resolve();
        }, 1000);
      });
    ...
    .then(() => {
      return new Promise((resolve) => {
        console.log(
          `${obtFechaHoraActual()} 📧 Enviando correo de compra al usuario.`
        );
        setTimeout(() => {
          console.log(
            `${obtFechaHoraActual()} 📧 Enviado correo de compra al usuario.`
          );
          resolve();
        }, 1000);
      });
    ...
    .then(() => {
      return new Promise((resolve) => {
        console.log(
          `${obtFechaHoraActual()} 🛒 Removiendo artículos del carrito de compras.`
        );
        setTimeout(() => {
    ...
    .then(() =>
      console.log(`${obtFechaHoraActual()} ✨ Compra completada con éxito.`)
    )
    .catch((error) =>
      console.log(`${obtFechaHoraActual()} 😱 Ha ocurrido un error: ${error}`)
    );
})(false);
